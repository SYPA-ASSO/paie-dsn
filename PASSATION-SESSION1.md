...
## 29. Invitation par e-mail a la creation d'un compte (lien de definition du mot de passe)

- REPONSE a la question de Francois : jusque-la AUCUN e-mail n'etait envoye a la creation d'un compte (mot de passe provisoire a transmettre par canal separe, jamais par e-mail : regle de securite). Desormais : chaque creation de compte (+ Nouveau client ET + Compte seul) genere un lien Supabase de type recovery (usage unique) et envoie via Brevo un e-mail d'invitation "Definissez votre mot de passe" pointant vers la NOUVELLE page /espace-client/nouveau-mot-de-passe (session echangee automatiquement par le client Supabase, formulaire nouveau mot de passe x2, gestion du lien expire avec repli connexion/contact).
...
## 31. Reponse a l'audit externe : securisation des ecrans-cles

Analyse externe recue par Francois (globalement positive, 4 critiques). Traitement :
1. PROMESSES ABSOLUES (accepte, corrige) : Hero badge "Toutes conventions collectives" -> "585 conventions collectives couvertes" (factuel, adosse au referentiel) ; "pas de veille a faire" -> "la veille juridique et conventionnelle est incluse" + mention "vous les validez" dans la phrase de process ; meta/OG layout et SchemaOrg : "bulletins conformes" -> "bulletins etablis selon votre convention collective", "Toutes conventions" -> "Referentiel des 585 conventions de branche en vigueur" ; PourQui ESS : suppression de "quelle que soit votre convention" et "Nous les connaissons" ; FAQ accueil et page conventions : reponse "Oui." absolue remplacee par la couverture chiffree du referentiel + cadrage conventionnel de reprise + identification des cas atypiques.
2. VALIDATION 48 H (defendue, mais annoncee en amont) : mecanisme legitime conserve ; NOUVELLE entree FAQ accueil "Comment mes bulletins sont-ils valides chaque mois ?" exposant le circuit complet (3 jours ouvres, relecture, 48 h, DSN a l'echeance) des la vitrine, plus de decouverte tardive en CGV.
3. HEBERGEMENT (accepte, precise) : politique de confidentialite reecrite par briques : site public Vercel (aucune donnee de paie, simulateur local), espace client et documents Supabase UE (authentifie, cloisonne par role), e-mails Brevo (societe francaise), paiements Stripe (donnees CB jamais accessibles au cabinet). A VERIFIER par Francois : region du projet Supabase (Dashboard -> Settings -> General -> Region) DOIT etre UE ; sinon me le signaler immediatement (migration a organiser + politique a reprendre).
4. PAGE RENONCIATION (accepte, refondue) : l'intro accrochait la resiliation a L. 221-21 (qui ne regit que l'exercice de la retractation) ; remplacee par deux encarts distincts : Retractation (L. 221-18, exercice en ligne L. 221-21, renonciation contenu numerique L. 221-28 13° renvoi CGV art. 12) / Resiliation (a tout moment, effet fin de periode, fonctionnalite en ligne L. 215-1-1). Plus aucun melange de regimes.
Non retenu : l'obligation de moyens et le cadre contractuel "serre" critiques comme decalage marketing/contrat : cadre classique et licite d'une prestation externalisee, desormais assume publiquement par la FAQ de validation.

## 32. Hebergement : cartographie precise validee par Francois

- Donnees confirmees par Francois : Vercel Inc. (site + fonctions applicatives) avec execution UE region PARIS et transferts encadres DATA PRIVACY FRAMEWORK UE-USA ; Supabase region IRLANDE (base, authentification). ATTENTION architecture paie-et-dsn : les documents sont dans le STORAGE SUPABASE (bucket prive), PAS sur Vercel Blob (le Blob region Paris concerne la Docutheque).
- Politique de confidentialite et mentions legales mises a jour avec ces regions et le DPF ; la FAQ (donnees hebergees dans l'UE) est desormais integralement substantielle. Ecart releve par l'audit externe : SOLDE.

## 33. Second retour externe (conversion) : arbitrages et actions

- Point 1 TROUVABILITE : artefact ("parked domain Hostinger" = fiche annuaire d'avant la mise en ligne, se rafraichira seule) + indexation mecaniquement jeune (domaine en prod depuis quelques jours, sitemap GSC en file). RIEN de casse (canonique apex, redirection www, sitemap 200 verifie). Plan inchange : propriete GSC de domaine, inspections d'URL, Bing import, patience. Optionnel entite : page LinkedIn (slogan livre), profils du cabinet pointant paie-et-dsn.fr.
- Point 2 SOUS-PONDERATION sur cholez-pagotto.fr (accepte, HORS PERIMETRE de ce repo) : a ajouter au prompt de la session Docutheque : creer une entree de service "Paie externalisee" visible (nav ou hub pros + encart associations) menant a paie-et-dsn.fr avec message d'atterrissage (variables -> bulletins -> DSN, pour qui, CTA devis), presente comme verticale du cabinet et non comme lien secondaire de la page ESS.
- Point 3 ENDOSSEMENT (accepte, FAIT) : le hero porte desormais "Un service du Cabinet Cholez-Pagotto, juriste en formalites et information juridique" avec lien cholez-pagotto.fr (le footer l'avait deja).
- Point 4 NETTETE OPERATIONNELLE (accepte, FAIT) : trois nouvelles FAQ vitrine : reprise de dossier (historique, cumuls, chiffree au devis, bascule 1er janvier ou en cours d'annee), evenements du mois (entree/DPAE, arret/signalement 5 j + IJSS + subrogation, sortie/STC), DSN rejetee (lecture mensuelle des retours organismes, correction avant l'echeance suivante, inclus sans supplement).

## 34. Grille de reprise de dossier : formalisee et integree

- GRILLE (validee par Francois, livree en grille-reprise-de-dossier.md) : forfait d'ouverture 120 EUR (particulier employeur 60 EUR ; OFFERT des 5 salaries si bascule au 1er janvier) + 12 EUR par salarie repris + cadrage conventionnel initial 90 EUR une seule fois par convention non outillee (capitalise en bibliotheque interne) + bulletins de reconstitution 8 EUR piece pour toute bascule en cours d'annee (ressaisie obligatoire depuis le 1er janvier, contrainte cumuls/reduction generale documentee par macompta.fr, FAQ 000 Changement de logiciel). Mois de rodage : premier bulletin compare ligne a ligne au dernier de l'ancien prestataire.
- SIMULATEUR : nouvelles questions (bascule 1er janvier / en cours d'annee + mois), ligne "Reprise de votre dossier (une seule fois)" calculee sous le forfait mensuel, mentions du forfait offert, du nombre de bulletins reconstitues et du cadrage conventionnel eventuel (non calcule, precise au devis). Constantes dans le composant : FORFAIT_OUVERTURE 120, REPRISE_PAR_SALARIE 12, BULLETIN_RECONSTITUTION 8. NOTE : le forfait particulier employeur 60 EUR n'est PAS dans le simulateur (pas de question "particulier employeur" a ce stade), il reste au devis.
- FAQ reprise : chiffres de la grille exposes + renvoi au simulateur + mois de rodage.
- Verification croisee des exemples de la grille contre le calcul du composant : 168 / 424 / 690 (600 + 90) OK.

## 35. Article blog : code APE et convention collective (23e article)

- Nouveau slug code-ape-convention-collective-erreur-paie (RH, 6 juillet 2026), ajoute a lib/blog-contenu-3.ts, inspire d'un article Village de la Justice signale par Francois mais INTEGRALEMENT reecrit, complementaire de trouver-sa-convention-collective-idcc (angle : le raccourci APE=CCN specifiquement ; presomption d'applicabilite par mention au bulletin, valeur contractuelle de la mention au contrat, methode en 4 verifications).
- Consigne de Francois appliquee : PAS de valorisation de l'IA ; valorisation du CABINET strictement dans son perimetre legal : la verification du rattachement conventionnel presentee comme premier acte technique de toute reprise de dossier de paie (information juridique accessoire a la prestation, coherent art. 60 loi 71-1130), avec ORIENTATION EXPLICITE VERS UN AVOCAT pour les qualifications litigieuses (renvoi /notre-perimetre). Maillage : trouver-sa-convention, referentiel conventions, comparatif, perimetre. Sitemap automatique (URL derivee du contenu). Blog : 23 articles.

## 36. Google Analytics installe SOUS CONDITION DE CONSENTEMENT (id G-PB1V59BK7B)

- ATTENTION CONFORMITE : la politique de confidentialite affirmait jusqu'ici "aucun cookie de suivi, aucun outil de mesure d'audience tiers" (point audite comme force, dispense de bandeau). L'installation demandee par Francois change cette posture : GA depose des cookies de mesure, consentement CNIL prealable obligatoire. Implementation choisie : PAS de balise brute dans le head (aurait ete non conforme), mais composant CookieBanner.tsx charge via next/script UNIQUEMENT apres clic "Accepter" (choix trace en localStorage cle paie-et-dsn-consentement-mesure), gtag config avec anonymize_ip true. "Refuser" = bandeau ferme, rien ne charge. Verifie au build : script googletagmanager ABSENT du HTML statique genere (index.html), donc bien conditionne cote client uniquement.
- Lien footer "Gerer mes cookies" (evenement custom ouvrir-preferences-cookies) permettant de rouvrir le bandeau et changer d'avis sans devoir vider le navigateur.
- Politique de confidentialite reecrite en consequence (GA avec consentement prealable, IP anonymisees, cookies d'authentification distincts sans consentement requis).
- A VERIFIER par Francois cote Google : proprietaire du compte GA G-PB1V59BK7B (probablement le meme que cholez-pagotto.fr, a confirmer), objectifs/evenements a configurer si besoin.
